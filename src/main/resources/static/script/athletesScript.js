...
function displayAndHide(section){
    const menuItems = ['home', 'classifications', 'about', 'myEnrollments', 'competitors', 'enrollment'];
                ...
                for(let i = 0; i<data['numberOfElements']; i++){
                    ...
                    switch(events[i]['eventStatus']){
                        ...
                        default: //for current or present events, the "Enroll" button is available
                            ...
                            secondButton.addEventListener("click", ()=> {enroll(events[i]['id'])});
            ...
            if(data){//if user is already enrolled in this event
                ...
                $.get("/api/v1/getAppUserByUsername", {username: user}, function (d) {
                    ...
                    Competitive Category: ${d.competitiveCategory}<br>`;
                        ...
                        cancelButton.textContent = "Cancel enrollment.";
                        confirmButton.setAttribute("id", "confirmEnrollment");
                        cancelButton.setAttribute("id", "cancelEnrollment");
                        confirmButton.addEventListener("click", ()=>{confirmEnrollment(eventId)});
                        cancelButton.addEventListener("click", cancelEnrollment);

                        let enrollment = document.getElementById("enrollmentDetails");
                        enrollment.innerHTML = "";
                        enrollment.appendChild(eventName);
                        enrollment.appendChild(enrollmentDetails);
                        enrollment.appendChild(confirmButton);
                        enrollment.appendChild(cancelButton);
                        displayAndHide('enrollment');
                    });
                });
            }
        });
...
function confirmEnrollment(eventId){
                ...
                alert("User already enrolled in this event!");
                    ...
                    $.get("/api/v1/getAppUserByUsername", {username: user}, function(d) {} //get user details for enrollment
                        ...
                        $.ajax({
                            ...
                            let paymentReference = jqXHR.responseText;
                            let paymentEntity = 12543;
